...
export default function Signup({ route, navigation }) {
        ...
        .then(res => {
          ...
          if (res?.data) {
            updateState({
              ...
              additionalSelectors:res?.data?.documents?.filter((x)=>x?.file_type == 'selector'),
  ...
  const _onSignup = () => {
    ...
    if (!isEmpty(additionalSelectors)) {
      additionalSelectors.map((i) => {
        if (!!i?.selectedValue) {
          formdata.append(i?.name, i?.selectedValue?.driver_registartion_option_name);
  ...
  const getAdditionalSelector=(item,index)=>{
    ...
    return(
      ...
      <DropDown
        value={item?.selectedValue?.driver_registartion_option_name ? item?.selectedValue?.driver_registartion_option_name:''}
        modalStyle={{
                      ...
                      if (vehicleTypes?.includes(i?.id)) {
                        return (
                          <TouchableOpacity
                            ...
                            <Image
                             style={i?.id == 6 ? {height: moderateScale(30), width:moderateScale(30), resizeMode:'contain'} : {}}
                              source={
                                selectedVehicleType == i
                                  ? i.activeIcon
                                  : i.inactiveIcon
