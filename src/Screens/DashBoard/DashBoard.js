...
import locationUpdateQueue from '../../utils/locationUpdateQueue';
...
export default function DashBoard({route, navigation}) {
  ...
  useEffect(() => {
    // Initialize location update queue
    locationUpdateQueue.init();
    ...
    .then(result => {
      ...
      return () => {
        ...
        // Clean up location queue
        locationUpdateQueue.destroy();
      };
    })
  ...
  const fetchgentLogs = async (lat, lng, heading_) => {
    if (userData?.access_token) {
      ...
      const headers = {client: clientInfo?.database_name};
      
      actions
        .logsApi(data, headers)
        .then(res => {
          // Check if the request failed
          if (res?.error) {
            console.log('Location update failed, will retry later');
            // Store failed update for retry
            if (locationUpdateQueue) {
              locationUpdateQueue.addToQueue(data, headers);
            }
            return;
          }
          ...
          // Only update app state if request was successful
          if (res?.data?.user?.client_preference) {
        ...
        .catch(error => {
          console.log('Error in fetchgentLogs:', error);
          // Store failed update for retry
          if (locationUpdateQueue) {
            locationUpdateQueue.addToQueue(data, headers);
          }
